...
module radix4_testbench;
  ...
  initial begin
    ...
    coeffs[2].im = 16'h2000;  // 0.5
    coeffs[3].re = 16'h1000;  // 0.25
    coeffs[3].im = 16'h0800;  // 0.125

    #10;

    $display("Result[0].re = %f", results[0].re / 16384.0);
    $display("Result[0].im = %f", results[0].im / 16384.0);
    $display("Result[1].re = %f", results[1].re / 16384.0);
    $display("Result[1].im = %f", results[1].im / 16384.0);
    $display("Result[2].re = %f", results[2].re / 16384.0);
    $display("Result[2].im = %f", results[2].im / 16384.0);
    $display("Result[3].re = %f", results[3].re / 16384.0);
    $display("Result[3].im = %f", results[3].im / 16384.0);
